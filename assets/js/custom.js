...
$(document).ready(function () {
  ...
  let cards = $('.popular-list .common-column');
  cards.slice(5).hide();

  $('.show__more').on('click', function() {
    $(this).parents('.popular-list').toggleClass('is-visible');
    if($(this).parents('.popular-list').hasClass('is-visible')){
      $(this).parents('.popular-list').find('.common-column').show();
      $(this).find('p').text('Show Less');
    } else {
      let cards = $(this).parents('.popular-list').find('.common-column');
      cards.slice(5).hide();
